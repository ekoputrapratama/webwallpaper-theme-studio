...
export function scratchProjectDir(id: string): string {
  return isRemote()
    ? path.join(os.tmpdir(), "webwallpaper-studio-scratch", id)
    : projectDir(id);
}
...
export async function newProjectId(): Promise<string> {
  if (isRemote()) {
    // Firestore's built-in auto document id — globally unique, never derived
    // from the name, so two projects can never share an id.
    return getDb().collection("projects").doc().id;
  }
  let id = "";
  do {
...
export async function hydrateProjectDir(
  id: string,
  dir: string,
): Promise<void> {
  if (!isRemote()) return;
...
export async function updateMeta(
  ...
  const meta = {
    ...(JSON.parse(fs.readFileSync(pj, "utf8")) as ThemeMeta),
    ...patch,
    updatedAt: new Date().toISOString(),
  };
    ...
    const walk = (rel: string) => {
      ...
      if (st.isDirectory()) {
        for (const child of fs.readdirSync(abs))
          walk(rel ? path.join(rel, child) : child);
